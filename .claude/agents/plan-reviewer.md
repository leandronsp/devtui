...
### 2. Find Gaps
- Missing error states (what if parsing fails? config missing? template not found?)
- Untested paths (what about empty input? malformed frontmatter? missing date field?)
- Editor edge cases (empty buffer, rapid mode switching, unicode, long lines)
- Engine edge cases (posts without description, lists without blank lines, emoji shortcodes)
